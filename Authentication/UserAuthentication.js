...
const UpdateUser=async(req,res,next)=>{
    ...
    if(user){
        try{
            ...
            req.session.user=user;
            user=await UserModel.findOne({_id:req.body._id})
            res.json({success:true, data:user,msg:"successfully updated"})
        }
        catch(err){
...
const loginUser=async(req,res,next)=>{
    ...
    if(user){
        if(await user.isPasswordCorrect(req.body.password)){
            ...
            res.cookie("auth",user._id,{httpOnly:true}).json({success:true,msg:"logged in successfully",token:token,data:user})
        }
        else{
